...
typedef struct {
    fsd_simple_config_t cfg;
    bool     cycle_active;
    uint16_t peak;
    uint16_t over_count;
    uint8_t  zero_run;
} fsd_simple_detector_t;

static inline void fsd_simple_default_config(fsd_simple_config_t* c,
        float thr_over_v, float min_peak_v,
        uint16_t min_over_count, uint8_t zeros_for_boundary)
{
